...
for hour in {0..23}; do
  for quarter in 0 15 30 45; do
    ...
    # Connectivity: up most of the time, down occasionally
    # Values must match the app's ConnectivityStatus enum (up/down/degraded)
    connectivity="up"
...
for hour in 0 4 8 12 16 20; do
  ...
  curl -G "${DB_URL}/exec" --data-urlencode "query=INSERT INTO network_metrics (
    ...
    '${timestamp}', 'speedtest', 'speedtest.net', null, null, null, 'up',
...
# QuestDB's /exec returns the aggregate value in "dataset":[[N]] (the top-level
# "count" field is the number of rows returned, which is always 1 here).
extract_count() {
  sed -n 's/.*"dataset":\[\[\([0-9]*\)\].*/\1/p'
}
ping_count=$(curl -sG "${DB_URL}/exec" --data-urlencode "query=SELECT COUNT(*) FROM network_metrics WHERE source = 'ping';" 2>/dev/null | extract_count)
ping_count=${ping_count:-0}
speedtest_count=$(curl -sG "${DB_URL}/exec" --data-urlencode "query=SELECT COUNT(*) FROM network_metrics WHERE source = 'speedtest';" 2>/dev/null | extract_count)
speedtest_count=${speedtest_count:-0}
